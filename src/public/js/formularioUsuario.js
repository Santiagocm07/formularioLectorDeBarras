...
document.querySelector('.loginUser').addEventListener('click', function() {
    const login = document.getElementById('usuario').value;
    const correctLogin = 'servicios.admin.autosLeo';

    const password = document.getElementById('contraseña').value;
    const correctPassword = 'almacen6151732';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function procesarMatricula(datos) {
        ...
        //ejemplo2
        const datosVehiculo = formattedData.split(/\s+/); // Separar por espacios simples

        if (datosVehiculo.length >= 4) {
            const codigoDepartamento = datosVehiculo.pop(); // Último elemento como código
            ...
            const indiceDireccion = datosVehiculo.findIndex(element => abreviaturas.includes(element));

            if (indiceDireccion !== -1) {
                nombrePropietario = datosVehiculo.slice(0, indiceDireccion).join(' '); // Todo antes de la dirección
                direccion = datosVehiculo.slice(indiceDireccion).join(' '); // Todo desde la dirección en adelante
            } else {
                // Si no se encuentra ninguna abreviatura, manejar la separación normalmente
                nombrePropietario = datosVehiculo.slice(0, -2).join(' '); // Todos menos los dos últimos
                direccion = datosVehiculo.slice(-2).join(' '); // Los últimos dos elementos como dirección
            }
